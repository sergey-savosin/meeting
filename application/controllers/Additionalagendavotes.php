...
class Additionalagendavotes extends MY_Controller {
	...
	public function index() {
		...
		if (1==1 && $this->input->post()) {
			...
			// $this->log_debug('Additionalagendavotes/index', json_encode($postdata));
			//ob_start();
			//var_dump($postdata);
			//$result = ob_get_clean();
			//$this->log_debug('Additionalagendavotes/index', $result);
		} else {
			$this->log_debug('Additionalagendavotes/index', 'no POST data');
		}
		...
		$stage_state = $this->Projects_model->getStageStatus($project_id, $current_date, 'additional_agenda');
		$this->log_debug('stage_state', $stage_state);

		// data for view
		$page_data['questions_query'] = 
			$this->Answers_model->fetch_additional_answers_with_votes($project_id, $user_id);
		$page_data['opened_questions_count'] =
			$this->Answers_model->get_opened_user_questions_count($user_id, 2); // additional questions
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->load->view('additionalagendavotes/view', $page_data);
